...
Method: Playwright evaluation of default-visible text in page body, excluding site chrome (header/footer), `aria-hidden`, and closed `<details>` content. Same method as production audit baseline (`audit/_metrics.json`).
...
| **After** (local prototype, reduced-motion) |              **338** | `toolkit-gate/_after-metrics.json`                               |
...
- After count uses the Operations Room prototype on branch `feat/visual-first-agent-toolkit` via `pnpm build` + local static preview.
